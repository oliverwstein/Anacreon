...
var cell := Vector2.ZERO:
	set(value):
		...
		tileData = get_parent().get_node("TileMap").GetTile(cell)['data']
		_timer.start()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	elif event is InputEventMouseButton or event.is_action_pressed("ui_accept"):
